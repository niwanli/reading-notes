...
**现在面临的问题：**
移动应用变得越来越复杂，需要提高电池的寿命和应用程序的性能。

**解决办法：**
利用移动边缘计算的计算迁移特性。

**创新点：**
提出了一种迁移系统模型和一个创新性的架构（MVR），以贡献于移动边缘计算中的计算迁移。

### 1. Introduction

**问题现状：**
Cisco的一份报告显示，数据流量快速增涨，物联网连接的设备量巨大。然而，移动设备的电池发展跟不上步调。考虑到移动设备的惊人增涨，解决能源阻碍是移动工业界最大的挑战。

**已有的研究及其缺点和不足：**
已有的研究方向，比如通过软件优化、漏洞检测和硬件优化来增强设备能源效率，但是这些方法只是减轻了基于设备本身的过度的能源消耗。

**提出新的研究方向：**
“端云融合”提供了一种完全不同的技术方向来优化能源消耗。然后，简要介绍了“端云融合”的概念和MEC技术。接着，阐述了该方案的优越性和美好前景。最后，总结了解决问题的思路和解决方法。

### 2. Offloading in MVR

介绍分割程序和做决策的重要性：MEC通过联合移动设备和边缘云来支持计算迁移。迁移计算没有必要将所有的程序执行过程转移到边缘云上。总的来说，程序首先被分为许多模块，其中一些需要在本地运行。
介绍MVR的工作原理及其带来的好处。

* A. MVR system model
* B. High-level of the MVR architecture
* C. The process of MVR offloading
